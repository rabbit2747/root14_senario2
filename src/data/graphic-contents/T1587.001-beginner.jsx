...
export const subtitlesData = [
  { ko: "안녕하세요! 지금부터 컴퓨터의 기본 원리와 이를 악용하는 해킹에 대해 알아보겠습니다.", en: "Hello! Let's explore the basic principles of computers and how hackers exploit them.", vi: null, ar: null, ja: null, zh: null, hi: null },
  { ko: "컴퓨터는 크게 입력, 처리, 출력이라는 세 가지 단순한 과정으로 작동하는 기계입니다.", en: "A computer is a machine that operates through three simple processes: input, processing, and output.", vi: null, ar: null, ja: null, zh: null, hi: null },
  { ko: "컴퓨터는 영어를 모릅니다. 오직 전기의 흐름인 0과 1, 즉 이진수만을 이해하고 처리하죠.", en: "Computers don't understand English. They only process binary — 0s and 1s, the flow of electricity.", vi: null, ar: null, ja: null, zh: null, hi: null },
  { ko: "이러한 과정을 수행하기 위해 물리적인 부품들이 필요한데, 가장 중요한 하드웨어 3대장을 소개합니다.", en: "Physical components are needed for these processes. Let's meet the three most important hardware components.", vi: null, ar: null, ja: null, zh: null, hi: null },
  { ko: "첫 번째는 CPU입니다. 사람의 뇌처럼 컴퓨터의 모든 계산과 명령 처리를 아주 빠르게 담당합니다.", en: "First is the CPU. Like a human brain, it handles all calculations and command processing at incredible speed.", vi: null, ar: null, ja: null, zh: null, hi: null },
  { ko: "해커는 바로 이 CPU의 '다음에 할 일' 목록을 교묘하게 조작하여 악성코드를 강제 실행하게 만듭니다.", en: "Hackers cleverly manipulate the CPU's 'to-do list' to force-execute malicious code.", vi: null, ar: null, ja: null, zh: null, hi: null },
  { ko: "두 번째는 RAM입니다. 속도가 빠른 CPU를 위해 데이터를 임시로 펼쳐두는 넓은 작업대 역할을 하죠.", en: "Second is RAM. It serves as a wide workspace that temporarily holds data for the fast CPU.", vi: null, ar: null, ja: null, zh: null, hi: null },
  { ko: "해커는 꼬리를 밟히지 않기 위해, 이 작업대(RAM) 구석에만 악성코드를 숨겨 백신을 피하는 Fileless 기법을 사용합니다.", en: "To avoid detection, hackers use Fileless techniques — hiding malware only in RAM corners to evade antivirus.", vi: null, ar: null, ja: null, zh: null, hi: null },
  { ko: "세 번째는 스토리지(SSD)입니다. 전원이 꺼져도 데이터를 영구적으로 안전하게 보관하는 튼튼한 서랍장입니다.", en: "Third is Storage (SSD). A sturdy cabinet that permanently and safely stores data even when powered off.", vi: null, ar: null, ja: null, zh: null, hi: null },
  { ko: "최근의 랜섬웨어는 이 서랍장 속 소중한 데이터들에 강력한 자물쇠를 채워 거액의 돈을 요구합니다.", en: "Recent ransomware locks precious data in this cabinet with strong padlocks and demands huge ransoms.", vi: null, ar: null, ja: null, zh: null, hi: null },
  { ko: "이러한 부품들을 우리가 쉽게 다룰 수 있게 해주는 총괄 매니저가 바로 '운영체제(OS)'입니다.", en: "The general manager that lets us easily use these components is the 'Operating System (OS)'.", vi: null, ar: null, ja: null, zh: null, hi: null },
  { ko: "운영체제는 실행된 프로그램을 '프로세스'라는 단위로 관리하며 메모리와 일꾼들을 배분합니다.", en: "The OS manages running programs as 'processes' and allocates memory and workers to each.", vi: null, ar: null, ja: null, zh: null, hi: null },
  { ko: "중요한 점은, 일반 프로그램은 '유저 모드'에서만 동작하며 시스템 통제 권한이 없다는 것입니다.", en: "Importantly, regular programs run only in 'User Mode' without system control privileges.", vi: null, ar: null, ja: null, zh: null, hi: null },
  { ko: "하지만 루트킷(Rootkit) 같은 고급 악성코드는 운영체제의 취약점을 파고들어 사장님 권한인 '커널 모드'를 탈취해 시스템을 완전히 장악합니다.", en: "But advanced malware like Rootkits exploit OS vulnerabilities to seize 'Kernel Mode' — the boss-level privileges — and take full control.", vi: null, ar: null, ja: null, zh: null, hi: null },
  { ko: "스토리지에 저장된 파일들은 확장자를 통해 종류가 구분되지만, 사실 이는 쉽게 속일 수 있습니다.", en: "Files in storage are identified by extensions, but these can be easily faked.", vi: null, ar: null, ja: null, zh: null, hi: null },
  { ko: "따라서 보안 분석가들은 확장자 대신, 파일 내부의 진짜 DNA인 '매직 넘버(Magic Number)'를 직접 확인합니다.", en: "So security analysts check the file's true DNA — the 'Magic Number' — instead of trusting the extension.", vi: null, ar: null, ja: null, zh: null, hi: null },
  { ko: "컴퓨터가 외부와 통신하려면, 건물 주소인 'IP 주소'와 방 번호인 '포트 번호'가 필요합니다.", en: "For a computer to communicate externally, it needs an 'IP address' (building address) and a 'Port number' (room number).", vi: null, ar: null, ja: null, zh: null, hi: null },
  { ko: "우리의 컴퓨터를 지켜주는 '방화벽'은 외부에서 들어오는 불법적인 접근 시도를 든든하게 막아줍니다.", en: "The 'Firewall' protecting our computer firmly blocks unauthorized external access attempts.", vi: null, ar: null, ja: null, zh: null, hi: null },
  { ko: "하지만 악성코드는 내부에서 해커의 서버(C2)로 먼저 전화를 거는 '리버스 쉘' 기법으로 방화벽을 무사히 통과합니다.", en: "But malware bypasses the firewall using 'Reverse Shell' — calling out to the hacker's C2 server from inside.", vi: null, ar: null, ja: null, zh: null, hi: null },
  { ko: "지금까지 컴퓨터의 구조와 해킹의 원리였습니다. 원리를 이해하면 방어할 수 있습니다. 수고하셨습니다!", en: "That covers computer architecture and hacking principles. Understanding the principles is the first step to defense. Great work!", vi: null, ar: null, ja: null, zh: null, hi: null },
];
