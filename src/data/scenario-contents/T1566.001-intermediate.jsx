...
const T = {
  ...
  zh: {
    loc1: 'Sector 1: Sysmon监控室', loc2: 'Sector 2: 规则引擎室', loc3: 'Sector 3: SIEM分析室', loc4: 'Sector 4: 取证实验室', loc5: 'Sector 5: 策略部署室',
    d1_1: "咳咳！{AGENT}特工，今天我们进入更高级的训练。你将学习如何使用Sysmon追踪鱼叉式网络钓鱼附件执行后留下的痕迹。",
    d1_2: "Sysmon？我听说它比标准的Windows事件日志更精确！",
    d1_3: "没错。Sysmon能详细记录进程创建、网络连接和文件变更。去监控室收集3个关键事件线索！",
    d2_1: "收集完毕！我用Sysmon Event ID 1捕获到了恶意附件生成cmd.exe作为子进程的痕迹。",
    d2_2: "很好。现在告诉我，实时记录进程创建的Sysmon Event ID是几号。",
    d3_1: "既然已经用Sysmon捕获了痕迹，现在移动到规则引擎室检测恶意软件本身。在这里你将学习编写YARA规则。",
    d3_2: "YARA？那是用来捕获隐藏在恶意软件二进制文件中的独特字符串和HEX模式的工具！",
    d3_3: "正确。YARA规则由三个部分组成：meta、strings和condition。去规则引擎室收集关键线索！",
    d4_1: "我理解了YARA规则的结构！在strings部分定义恶意软件特有的字符串，在condition中设置检测条件。",
    d4_2: "好。现在告诉我这个检测恶意软件二进制模式的规则引擎的确切名称。",
    d5_1: "如果说YARA是文件级检测，那SIEM就是日志级检测。它将数千台设备的日志汇聚到一处进行关联分析。",
    d5_2: "但每个SIEM产品的查询语法都不同。Splunk用SPL，Elastic用KQL……有没有统一的检测规则？",
    d5_3: "Sigma正是为此而生！用供应商中立的YAML格式编写一次，就能转换到任何SIEM使用。去分析室收集线索！",
    d6_1: "用Sigma规则定义'可疑的父子进程关系'后，Splunk和Elastic都能实现相同的检测！",
    d6_2: "没错。现在告诉我SIEM系统中使用的这种供应商中立检测规则格式的名称。",
    d7_1: "检测规则就绪后，是时候追溯攻击源头了。鱼叉式网络钓鱼始于电子邮件，所以我们需要分析邮件头。",
    d7_2: "邮件头除了发件人和收件人，还包含邮件经过的服务器路径，对吧？",
    d7_3: "对。从下往上读取Received头可以确定最初的发送服务器。还需要检查X-headers中的SPF和DKIM验证结果。去取证实验室收集线索！",
    d8_1: "我反向追踪了Received链并确定了发送服务器IP！SPF fail、DKIM none——这确实是伪造发件人域名的鱼叉式网络钓鱼邮件。",
    d8_2: "很好。现在告诉我能够追踪邮件服务器路径的头字段的确切名称。",
    d9_1: "这是最后阶段。完成检测和分析后，我们需要在整个组织部署防御策略。我们将学习GPO、ASR和三轴防御模型。",
    d9_2: "GPO在组织范围内部署组策略，ASR通过阻止Office宏执行等来减少攻击面，对吧？",
    d9_3: "没错。三轴防御模型——技术(Technology)、流程(Process)、人员(People)——三个轴必须保持平衡才能实现真正的防御。去策略部署室收集剩余线索！",
    d10_1: "我已经通过GPO在全组织部署了宏阻止策略，并用ASR规则从根源阻止了可疑子进程的创建！",
    d10_2: "好。现在告诉我阻止Office宏执行的Windows安全策略的确切名称。",
    d11_1: "所有区域的探索和策略部署已完成！我系统地学习了Sysmon监控、YARA/Sigma检测、邮件取证以及GPO/ASR策略。",
    d11_2: "辛苦了，{AGENT}。你出色地完成了中级课程。今天的学习内容已记录在最终案件笔记中——务必仔细复习。",
    h1: "点击Sysmon事件监视器(Activity)、进程树(GitBranch)、<br>以及事件日志文件(FileText)。",
    h2: "点击YARA规则编辑器(Code)、HEX模式查看器(Binary)、<br>以及字符串匹配扫描器(Search)。",
    h3: "点击Sigma规则编辑器(Shield)、SIEM告警面板(AlertTriangle)、<br>以及关联分析链接(Link)。",
    h4: "点击邮件头分析器(Mail)、Received链追踪器(Route)、<br>以及X-Header验证查看器(Eye)。",
    h5: "点击GPO策略编辑器(Settings)、ASR规则面板(ShieldCheck)、<br>以及三轴防御模型图(Target)。",
    c_sysmon_t: "Sysmon事件", c_sysmon_m: "Sysmon(System Monitor)是Windows高级监控工具，可精确记录进程创建(ID 1)、网络连接(ID 3)、文件创建(ID 11)等。",
    c_proc_t: "进程树", c_proc_m: "以树形结构可视化父子进程关系。检测WINWORD.EXE → cmd.exe → powershell.exe等可疑链的关键技术。",
    c_event_t: "事件日志", c_event_m: "记录在Windows事件查看器中的结构化日志。从Sysmon通道(Microsoft-Windows-Sysmon/Operational)过滤安全相关事件。",
    c_yara_t: "YARA规则", c_yara_m: "定义恶意软件独特模式的规则引擎。由meta(元数据)、strings(检测字符串)、condition(条件)三个部分组成，用于文件扫描。",
    c_hex_t: "HEX模式", c_hex_m: "二进制文件内部的十六进制字节序列。在YARA的strings部分定义{ 4D 5A 90 00 }等HEX模式来检测PE头等。",
    c_string_t: "字符串匹配", c_string_m: "在YARA strings中定义恶意软件特有字符串(C2域名、注册表键、API调用名)，无需静态分析即可快速检测。",
    c_sigma_t: "Sigma规则", c_sigma_m: "供应商中立的YAML格式检测规则。编写一次后可通过sigmac编译器自动转换为Splunk SPL、Elastic KQL、Microsoft KQL等。",
    c_siem_t: "SIEM告警", c_siem_m: "SIEM(Security Information and Event Management)通过关联分析生成的安全告警。分析多个事件之间的关联性，而非单一事件。",
    c_corr_t: "关联分析", c_corr_m: "SIEM的核心功能，将不同日志源(Sysmon、防火墙、IDS)的事件按时间序列连接，重建攻击链。",
    c_email_t: "邮件头", c_email_m: "电子邮件的元数据。除了标准的From、To、Subject外，还包含Received、Message-ID、X-Mailer及认证信息。",
    c_recv_t: "Received链", c_recv_m: "邮件经过的邮件服务器(MTA)路径。从下往上读取可追溯：源服务器 → 中继服务器 → 最终接收服务器。",
    c_xhdr_t: "X-Header分析", c_xhdr_m: "X-Spam-Status、Authentication-Results等扩展头。用于验证SPF pass/fail、DKIM签名验证及DMARC策略应用情况。",
    c_gpo_t: "GPO策略", c_gpo_m: "Group Policy Object——通过Active Directory向域内所有计算机批量部署安全策略的Windows管理工具。",
    c_asr_t: "ASR规则", c_asr_m: "Attack Surface Reduction——Microsoft Defender的攻击面缩减规则，可从源头阻止Office宏子进程创建、混淆脚本执行等。",
    c_def_t: "三轴防御", c_def_m: "安全框架，认为技术(Technology)、流程(Process)、人员(People)三个轴必须保持平衡才能实现完整防御。",
    p1_title: "Sysmon事件追踪", p1_desc: "哪个Sysmon Event ID实时记录<b>进程创建</b>？",
    p1_opts: ["Event ID 1", "Event ID 3", "Event ID 7", "Event ID 11"],
    p2_title: "二进制检测引擎", p2_desc: "哪个规则引擎检测<b>恶意软件中的二进制模式</b>？",
    p2_opts: ["Snort", "YARA", "Sigma", "Suricata"],
    p3_title: "供应商中立检测规则", p3_desc: "SIEM系统中使用的<b>供应商中立检测规则格式</b>是什么？",
    p3_opts: ["YARA", "Snort", "Sigma", "KQL"],
    p4_title: "邮件路径追踪", p4_desc: "哪个头字段可以<b>追踪邮件经过的服务器路径</b>？",
    p4_opts: ["Received", "From", "Reply-To", "Return-Path"],
    p5_title: "攻击面缩减", p5_desc: "哪个Windows安全策略<b>阻止Office宏执行</b>？",
    p5_opts: ["UAC", "ASR", "BitLocker", "AppLocker"],
    ...
    rTab1: "1. Sysmon与进程追踪", rTab2: "2. YARA检测规则", rTab3: "3. Sigma与SIEM",
    rTab4: "4. 邮件取证", rTab5: "5. 基于策略的防御", rTab6: "6. 训练完成",
    r1_h: "Sysmon与进程追踪",
    r1_ch1: "第1章 Sysmon Event ID的作用",
    r1_ch1_l1: '<span class="text-white bg-white/10 px-1 rounded">Event ID 1</span> — 进程创建。记录执行了什么程序及其父进程。',
    r1_ch1_l2: '<span class="text-white bg-white/10 px-1 rounded">Event ID 3</span> — 网络连接。检测与C2服务器的通信尝试。',
    r1_ch1_l3: '<span class="text-white bg-white/10 px-1 rounded">Event ID 11</span> — 文件创建。追踪投放的恶意载荷。',
    r1_ch2: "第2章 进程树分析",
    r1_ch2_l1: '像WINWORD.EXE → cmd.exe → powershell.exe这样的异常父子关系是<span class="text-white bg-white/10 px-1 rounded">鱼叉式网络钓鱼的典型痕迹</span>。',
    r1_ch2_l2: '正常的Office进程不会创建子进程，因此这是一个可靠的检测标准。',
    r2_h: "YARA检测规则",
    r2_ch3: "第3章 YARA规则结构 (meta/strings/condition)",
    r2_ch3_l1: '<span class="text-white bg-white/10 px-1 rounded">meta</span> — 定义元数据，如规则作者、描述和参考URL。',
    r2_ch3_l2: '<span class="text-white bg-white/10 px-1 rounded">strings</span> — 定义检测字符串、HEX模式和正则表达式。',
    r2_ch3_l3: '<span class="text-white bg-white/10 px-1 rounded">condition</span> — 设置需要匹配多少模式才触发检测。',
    r2_ch4: "第4章 HEX模式匹配",
    r2_ch4_l1: '为PE文件的<span class="text-white bg-white/10 px-1 rounded">MZ头(4D 5A)</span>或恶意shellcode的独特字节序列定义HEX模式。',
    r2_ch4_l2: '通配符(??)、跳转([2-4])、替代(A|B)等高级模式语法可检测变种。',
    r3_h: "Sigma与SIEM",
    r3_ch5: "第5章 Sigma规则语法",
    r3_ch5_l1: '以<span class="text-white bg-white/10 px-1 rounded">YAML格式</span>编写，由title、logsource、detection和condition字段组成。',
    r3_ch5_l2: '可通过sigmac<span class="text-white bg-white/10 px-1 rounded">自动转换</span>为Splunk SPL、Elastic KQL、Microsoft Sentinel等。',
    r3_ch6: "第6章 SIEM关联分析",
    r3_ch6_l1: '将<span class="text-white bg-white/10 px-1 rounded">多个日志源</span>(Sysmon + 防火墙 + IDS)按时间序列连接，而不仅是单一事件。',
    r3_ch6_l2: '将邮件接收 → 附件执行 → C2通信尝试重建为单一攻击链进行检测。',
    r4_h: "邮件取证",
    r4_ch7: "第7章 头字段含义",
    r4_ch7_l1: '<span class="text-white bg-white/10 px-1 rounded">From / Reply-To</span> — 发件人地址。可以伪造，切勿单独信任。',
    r4_ch7_l2: '<span class="text-white bg-white/10 px-1 rounded">Message-ID / X-Mailer</span> — 邮件客户端和唯一标识符。可检测群发工具的痕迹。',
    r4_ch7_l3: '<span class="text-white bg-white/10 px-1 rounded">Authentication-Results</span> — 验证SPF、DKIM和DMARC结果的关键头字段。',
    r4_ch8: "第8章 Received链反向追踪",
    r4_ch8_l1: '<span class="text-white bg-white/10 px-1 rounded">从下往上</span>读取Received头可揭示源服务器 → 中继服务器 → 接收服务器的路径。',
    r4_ch8_l2: '通过WHOIS和VirusTotal查询发送服务器IP以检查是否为恶意基础设施。',
    r5_h: "基于策略的防御",
    r5_ch9: "第9章 GPO与ASR",
    r5_ch9_l1: '<span class="text-white bg-white/10 px-1 rounded">GPO(Group Policy Object)</span> — 通过Active Directory在域范围内批量部署安全策略。',
    r5_ch9_l2: '<span class="text-white bg-white/10 px-1 rounded">ASR(Attack Surface Reduction)</span> — 从源头阻止Office宏子进程创建、混淆脚本执行等。',
    r5_ch10: "第10章 三轴防御模型",
    r5_ch10_l1: "技术(Technology) — 技术检测/阻止工具：Sysmon、YARA、Sigma、ASR",
    r5_ch10_l2: "流程(Process) — 事件响应流程、补丁管理、日志保留策略",
    r5_ch10_l3: "人员(People) — 安全意识培训、钓鱼模拟、SOC分析师能力提升",
    r6_title: "训练完成！",
    r6_desc: "从Sysmon监控到YARA/Sigma检测规则，<br/>邮件取证，以及GPO/ASR策略部署——<br/>你已完成中级鱼叉式网络钓鱼防御课程。",
    r6_grade: "最终评级：",
    r6_labBtn: "前往实验室",
    r6_freeHint: "(关闭此窗口可自由重新探索各区域)",
  },
  hi: {
    loc1: 'Sector 1: Sysmon नियंत्रण कक्ष', loc2: 'Sector 2: रूल इंजन लैब', loc3: 'Sector 3: SIEM विश्लेषण कक्ष', loc4: 'Sector 4: फोरेंसिक लैब', loc5: 'Sector 5: नीति तैनाती कक्ष',
    d1_1: "ध्यान दें! एजेंट {AGENT}, आज हम एक उच्च स्तर का प्रशिक्षण करेंगे। आप सीखेंगे कि Sysmon का उपयोग करके स्पीयरफ़िशिंग अटैचमेंट के निष्पादन से छोड़े गए निशानों को कैसे ट्रैक करें।",
    d1_2: "Sysmon? मैंने सुना है कि यह मानक Windows इवेंट लॉग से अधिक सटीक निगरानी उपकरण है!",
    d1_3: "सही। Sysmon प्रोसेस निर्माण, नेटवर्क कनेक्शन और फ़ाइल परिवर्तनों को विस्तार से रिकॉर्ड करता है। नियंत्रण कक्ष से 3 प्रमुख इवेंट सुराग एकत्र करें!",
    d2_1: "संग्रह पूर्ण! मैंने Sysmon Event ID 1 का उपयोग करके दुर्भावनापूर्ण अटैचमेंट द्वारा cmd.exe को चाइल्ड प्रोसेस के रूप में बनाने के निशान पकड़े।",
    d2_2: "उत्कृष्ट। अब मुझे बताएं कि वास्तविक समय में प्रोसेस निर्माण रिकॉर्ड करने वाला Sysmon Event ID कौन सा है।",
    d3_1: "Sysmon से निशान पकड़ने के बाद, अब रूल इंजन लैब में जाकर मैलवेयर का पता लगाना सीखें। यहाँ आप YARA नियम लिखना सीखेंगे।",
    d3_2: "YARA? वह उपकरण जो मैलवेयर बाइनरी में छिपे अद्वितीय स्ट्रिंग्स और HEX पैटर्न को पकड़ता है!",
    d3_3: "बिल्कुल सही। YARA नियम तीन ब्लॉक से बने होते हैं: meta, strings, और condition। रूल इंजन लैब से प्रमुख सुराग एकत्र करें!",
    d4_1: "मैंने YARA नियम संरचना समझ ली! strings अनुभाग में मैलवेयर-विशिष्ट स्ट्रिंग्स परिभाषित करें और condition में पहचान की शर्तें सेट करें।",
    d4_2: "अच्छा। अब मुझे मैलवेयर में बाइनरी पैटर्न का पता लगाने वाले इस रूल इंजन का सटीक नाम बताएं।",
    d5_1: "अगर YARA फ़ाइल स्तर पर पहचान करता है, तो SIEM लॉग स्तर पर पहचान करता है। यह हज़ारों उपकरणों से लॉग एकत्र करके सहसंबंध विश्लेषण करता है।",
    d5_2: "लेकिन हर SIEM उत्पाद की क्वेरी सिंटैक्स अलग होती है। Splunk SPL का उपयोग करता है, Elastic KQL का... क्या कोई एकीकृत पहचान नियम नहीं है?",
    d5_3: "Sigma बिल्कुल यही करता है! वेंडर-न्यूट्रल YAML प्रारूप में एक बार लिखें, किसी भी SIEM के लिए रूपांतरित करें। विश्लेषण कक्ष से सुराग एकत्र करें!",
    d6_1: "Sigma नियमों से 'संदिग्ध पैरेंट-चाइल्ड प्रोसेस संबंध' परिभाषित करने पर, Splunk और Elastic दोनों पर समान पहचान काम करती है!",
    d6_2: "सही। अब मुझे SIEM सिस्टम में उपयोग किए जाने वाले इस वेंडर-न्यूट्रल पहचान नियम प्रारूप का नाम बताएं।",
    d7_1: "पहचान नियम तैयार होने के बाद, हमले के मूल को ट्रैक करने का समय है। स्पीयरफ़िशिंग ईमेल से शुरू होती है, इसलिए हमें ईमेल हेडर का विश्लेषण करना होगा।",
    d7_2: "ईमेल हेडर में प्रेषक और प्राप्तकर्ता के अलावा ईमेल द्वारा तय किए गए सर्वर पथ भी होते हैं, है ना?",
    d7_3: "सही। Received हेडर को नीचे से ऊपर पढ़ने से मूल प्रेषण सर्वर की पहचान होती है। X-headers में SPF और DKIM सत्यापन परिणाम भी जाँचने होंगे। फोरेंसिक लैब से सुराग एकत्र करें!",
    d8_1: "मैंने Received चेन को उल्टा ट्रैक करके प्रेषण सर्वर IP की पहचान की! SPF fail, DKIM none — यह निश्चित रूप से जाली प्रेषक डोमेन वाली स्पीयरफ़िशिंग ईमेल है।",
    d8_2: "उत्कृष्ट। अब मुझे उस हेडर फ़ील्ड का सटीक नाम बताएं जो ईमेल के सर्वर पथ को ट्रैक करने देता है।",
    d9_1: "यह अंतिम चरण है। पहचान और विश्लेषण पूर्ण होने के बाद, हमें पूरे संगठन में रक्षा नीतियाँ तैनात करनी होंगी। हम GPO, ASR और त्रि-अक्ष रक्षा मॉडल सीखेंगे।",
    d9_2: "GPO संगठन-व्यापी समूह नीतियाँ तैनात करता है, और ASR Office मैक्रो निष्पादन जैसी चीज़ों को रोककर हमले की सतह कम करता है, है ना?",
    d9_3: "सही। और त्रि-अक्ष रक्षा मॉडल — प्रौद्योगिकी(Technology), प्रक्रिया(Process), लोग(People) — तीनों अक्षों को संतुलित होना चाहिए। नीति तैनाती कक्ष से शेष सुराग एकत्र करें!",
    d10_1: "मैंने GPO के माध्यम से पूरे संगठन में मैक्रो-ब्लॉकिंग नीतियाँ तैनात कीं और ASR नियमों से संदिग्ध चाइल्ड प्रोसेस निर्माण को मूल स्रोत पर रोक दिया!",
    d10_2: "अच्छा। अब मुझे Office मैक्रो निष्पादन को रोकने वाली Windows सुरक्षा नीति का सटीक नाम बताएं।",
    d11_1: "सभी सेक्टरों की खोज और नीति तैनाती पूर्ण! मैंने व्यवस्थित रूप से Sysmon निगरानी, YARA/Sigma पहचान, ईमेल फोरेंसिक और GPO/ASR नीतियाँ सीखीं।",
    d11_2: "बहुत अच्छा, {AGENT}। आपने मध्यवर्ती पाठ्यक्रम उत्कृष्ट रूप से पूरा किया। आज के पाठ अंतिम केस नोटबुक में दर्ज हैं — ध्यान से समीक्षा करें।",
    h1: "Sysmon इवेंट मॉनिटर (Activity), प्रोसेस ट्री (GitBranch), <br>और इवेंट लॉग फ़ाइल (FileText) पर क्लिक करें।",
    h2: "YARA रूल एडिटर (Code), HEX पैटर्न व्यूअर (Binary), <br>और स्ट्रिंग मैचिंग स्कैनर (Search) पर क्लिक करें।",
    h3: "Sigma रूल एडिटर (Shield), SIEM अलर्ट पैनल (AlertTriangle), <br>और कोरिलेशन लिंक (Link) पर क्लिक करें।",
    h4: "ईमेल हेडर एनालाइज़र (Mail), Received चेन ट्रैकर (Route), <br>और X-Header वेरिफ़िकेशन व्यूअर (Eye) पर क्लिक करें।",
    h5: "GPO पॉलिसी एडिटर (Settings), ASR रूल पैनल (ShieldCheck), <br>और त्रि-अक्ष रक्षा मॉडल आरेख (Target) पर क्लिक करें।",
    c_sysmon_t: "Sysmon इवेंट", c_sysmon_m: "Sysmon (System Monitor) Windows के लिए एक उन्नत निगरानी उपकरण है जो प्रोसेस निर्माण (ID 1), नेटवर्क कनेक्शन (ID 3), फ़ाइल निर्माण (ID 11) आदि को सटीक रूप से रिकॉर्ड करता है।",
    c_proc_t: "प्रोसेस ट्री", c_proc_m: "पैरेंट-चाइल्ड प्रोसेस संबंधों को ट्री रूप में दर्शाता है। WINWORD.EXE → cmd.exe → powershell.exe जैसी संदिग्ध श्रृंखलाओं का पता लगाने की प्रमुख तकनीक।",
    c_event_t: "इवेंट लॉग", c_event_m: "Windows इवेंट व्यूअर में रिकॉर्ड किए गए संरचित लॉग। Sysmon चैनल (Microsoft-Windows-Sysmon/Operational) से सुरक्षा संबंधित इवेंट फ़िल्टर किए जाते हैं।",
    c_yara_t: "YARA नियम", c_yara_m: "मैलवेयर के अद्वितीय पैटर्न परिभाषित करने वाला रूल इंजन। meta (मेटाडेटा), strings (पहचान स्ट्रिंग्स), और condition (शर्तें) तीन ब्लॉक से बना, फ़ाइल स्कैनिंग के लिए उपयोग होता है।",
    c_hex_t: "HEX पैटर्न", c_hex_m: "बाइनरी फ़ाइलों के अंदर हेक्साडेसिमल बाइट अनुक्रम। YARA के strings अनुभाग में { 4D 5A 90 00 } जैसे HEX पैटर्न परिभाषित करके PE हेडर आदि का पता लगाया जाता है।",
    c_string_t: "स्ट्रिंग मैचिंग", c_string_m: "YARA strings में मैलवेयर-विशिष्ट स्ट्रिंग्स (C2 डोमेन, रजिस्ट्री कुंजियाँ, API कॉल नाम) परिभाषित करके स्थैतिक विश्लेषण के बिना तेज़ पहचान।",
    c_sigma_t: "Sigma नियम", c_sigma_m: "वेंडर-न्यूट्रल YAML प्रारूप में पहचान नियम। एक बार लिखकर sigmac कंपाइलर से Splunk SPL, Elastic KQL, Microsoft KQL आदि में स्वतः रूपांतरित किए जा सकते हैं।",
    c_siem_t: "SIEM अलर्ट", c_siem_m: "SIEM (Security Information and Event Management) द्वारा सहसंबंध विश्लेषण के माध्यम से उत्पन्न सुरक्षा अलर्ट। एकल इवेंट नहीं बल्कि बहु-इवेंट संबंधों का विश्लेषण करता है।",
    c_corr_t: "सहसंबंध विश्लेषण", c_corr_m: "SIEM की मूल कार्यक्षमता जो विभिन्न लॉग स्रोतों (Sysmon, फ़ायरवॉल, IDS) के इवेंट को कालानुक्रमिक रूप से जोड़कर हमले की श्रृंखला का पुनर्निर्माण करती है।",
    c_email_t: "ईमेल हेडर", c_email_m: "ईमेल का मेटाडेटा। मानक From, To, Subject के अलावा Received, Message-ID, X-Mailer और प्रमाणीकरण जानकारी शामिल होती है।",
    c_recv_t: "Received चेन", c_recv_m: "ईमेल द्वारा तय किए गए मेल सर्वर (MTA) का पथ। नीचे से ऊपर पढ़ने पर पता चलता है: मूल सर्वर → रिले सर्वर → अंतिम प्राप्त सर्वर।",
    c_xhdr_t: "X-Header विश्लेषण", c_xhdr_m: "X-Spam-Status, Authentication-Results जैसे विस्तारित हेडर। SPF pass/fail सत्यापन, DKIM हस्ताक्षर सत्यापन और DMARC नीति अनुप्रयोग जाँचने के लिए उपयोग होते हैं।",
    c_gpo_t: "GPO नीति", c_gpo_m: "Group Policy Object — Active Directory के माध्यम से डोमेन में सभी कंप्यूटरों पर सुरक्षा नीतियाँ बैच-तैनात करने का Windows प्रबंधन उपकरण।",
    c_asr_t: "ASR नियम", c_asr_m: "Attack Surface Reduction — Microsoft Defender के हमले की सतह कम करने वाले नियम जो Office मैक्रो चाइल्ड प्रोसेस निर्माण, अस्पष्ट स्क्रिप्ट निष्पादन आदि को मूल स्रोत पर रोकते हैं।",
    c_def_t: "त्रि-अक्ष रक्षा", c_def_m: "सुरक्षा ढाँचा जो कहता है कि प्रौद्योगिकी (Technology), प्रक्रिया (Process), और लोग (People) — तीनों अक्षों को पूर्ण रक्षा के लिए संतुलित होना चाहिए।",
    p1_title: "Sysmon इवेंट ट्रैकिंग", p1_desc: "कौन सा Sysmon Event ID वास्तविक समय में <b>प्रोसेस निर्माण</b> रिकॉर्ड करता है?",
    p1_opts: ["Event ID 1", "Event ID 3", "Event ID 7", "Event ID 11"],
    p2_title: "बाइनरी पहचान इंजन", p2_desc: "कौन सा रूल इंजन <b>मैलवेयर में बाइनरी पैटर्न</b> का पता लगाता है?",
    p2_opts: ["Snort", "YARA", "Sigma", "Suricata"],
    p3_title: "वेंडर-न्यूट्रल पहचान नियम", p3_desc: "SIEM सिस्टम में उपयोग किया जाने वाला <b>वेंडर-न्यूट्रल पहचान नियम प्रारूप</b> क्या है?",
    p3_opts: ["YARA", "Snort", "Sigma", "KQL"],
    p4_title: "ईमेल पथ ट्रैकिंग", p4_desc: "कौन सा हेडर फ़ील्ड ईमेल द्वारा तय किए गए <b>सर्वर पथ को ट्रैक</b> करने देता है?",
    p4_opts: ["Received", "From", "Reply-To", "Return-Path"],
    p5_title: "हमले की सतह में कमी", p5_desc: "कौन सी Windows सुरक्षा नीति <b>Office मैक्रो निष्पादन को रोकती</b> है?",
    p5_opts: ["UAC", "ASR", "BitLocker", "AppLocker"],
    ...
    rTab1: "1. Sysmon और प्रोसेस ट्रैकिंग", rTab2: "2. YARA पहचान नियम", rTab3: "3. Sigma और SIEM",
    rTab4: "4. ईमेल फोरेंसिक", rTab5: "5. नीति-आधारित रक्षा", rTab6: "6. प्रशिक्षण पूर्ण",
    r1_h: "Sysmon और प्रोसेस ट्रैकिंग",
    r1_ch1: "अध्याय 1. Sysmon Event ID की भूमिकाएँ",
    r1_ch1_l1: '<span class="text-white bg-white/10 px-1 rounded">Event ID 1</span> — प्रोसेस निर्माण। कौन सा प्रोग्राम निष्पादित हुआ और उसका पैरेंट प्रोसेस क्या था, रिकॉर्ड करता है।',
    r1_ch1_l2: '<span class="text-white bg-white/10 px-1 rounded">Event ID 3</span> — नेटवर्क कनेक्शन। C2 सर्वर से संचार प्रयासों का पता लगाता है।',
    r1_ch1_l3: '<span class="text-white bg-white/10 px-1 rounded">Event ID 11</span> — फ़ाइल निर्माण। ड्रॉप किए गए दुर्भावनापूर्ण पेलोड को ट्रैक करता है।',
    r1_ch2: "अध्याय 2. प्रोसेस ट्री विश्लेषण",
    r1_ch2_l1: 'WINWORD.EXE → cmd.exe → powershell.exe जैसे असामान्य पैरेंट-चाइल्ड संबंध <span class="text-white bg-white/10 px-1 rounded">स्पीयरफ़िशिंग के विशिष्ट निशान</span> हैं।',
    r1_ch2_l2: 'सामान्य Office प्रोसेस चाइल्ड प्रोसेस नहीं बनाते, जो इसे एक विश्वसनीय पहचान मानदंड बनाता है।',
    r2_h: "YARA पहचान नियम",
    r2_ch3: "अध्याय 3. YARA नियम संरचना (meta/strings/condition)",
    r2_ch3_l1: '<span class="text-white bg-white/10 px-1 rounded">meta</span> — मेटाडेटा परिभाषित करता है जैसे नियम लेखक, विवरण और संदर्भ URL।',
    r2_ch3_l2: '<span class="text-white bg-white/10 px-1 rounded">strings</span> — पहचान स्ट्रिंग्स, HEX पैटर्न और रेगुलर एक्सप्रेशन परिभाषित करता है।',
    r2_ch3_l3: '<span class="text-white bg-white/10 px-1 rounded">condition</span> — पहचान के लिए कितने पैटर्न मैच होने चाहिए, सेट करता है।',
    r2_ch4: "अध्याय 4. HEX पैटर्न मैचिंग",
    r2_ch4_l1: 'PE फ़ाइल के <span class="text-white bg-white/10 px-1 rounded">MZ हेडर (4D 5A)</span> या दुर्भावनापूर्ण शेलकोड के अद्वितीय बाइट अनुक्रमों के लिए HEX पैटर्न परिभाषित करता है।',
    r2_ch4_l2: 'वाइल्डकार्ड (??), जंप ([2-4]), और विकल्प (A|B) जैसी उन्नत पैटर्न सिंटैक्स से वेरिएंट का पता लगाया जा सकता है।',
    r3_h: "Sigma और SIEM",
    r3_ch5: "अध्याय 5. Sigma नियम सिंटैक्स",
    r3_ch5_l1: '<span class="text-white bg-white/10 px-1 rounded">YAML प्रारूप</span> में लिखा जाता है, title, logsource, detection और condition फ़ील्ड से बना होता है।',
    r3_ch5_l2: 'sigmac के माध्यम से Splunk SPL, Elastic KQL, Microsoft Sentinel आदि में <span class="text-white bg-white/10 px-1 rounded">स्वतः रूपांतरित</span> किया जा सकता है।',
    r3_ch6: "अध्याय 6. SIEM सहसंबंध विश्लेषण",
    r3_ch6_l1: 'केवल एकल इवेंट नहीं बल्कि <span class="text-white bg-white/10 px-1 rounded">बहु लॉग स्रोतों</span> (Sysmon + फ़ायरवॉल + IDS) को कालानुक्रमिक रूप से जोड़ता है।',
    r3_ch6_l2: 'ईमेल प्राप्ति → अटैचमेंट निष्पादन → C2 संचार प्रयास को एकल हमला श्रृंखला के रूप में पुनर्निर्मित करके पहचान करता है।',
    r4_h: "ईमेल फोरेंसिक",
    r4_ch7: "अध्याय 7. हेडर फ़ील्ड का अर्थ",
    r4_ch7_l1: '<span class="text-white bg-white/10 px-1 rounded">From / Reply-To</span> — प्रेषक पता। जाली बनाया जा सकता है, इसलिए अकेले भरोसा न करें।',
    r4_ch7_l2: '<span class="text-white bg-white/10 px-1 rounded">Message-ID / X-Mailer</span> — मेल क्लाइंट और अद्वितीय पहचानकर्ता। सामूहिक मेलिंग उपकरणों के निशान का पता लगाता है।',
    r4_ch7_l3: '<span class="text-white bg-white/10 px-1 rounded">Authentication-Results</span> — SPF, DKIM और DMARC परिणाम सत्यापित करने का प्रमुख हेडर।',
    r4_ch8: "अध्याय 8. Received चेन बैकट्रैकिंग",
    r4_ch8_l1: 'Received हेडर को <span class="text-white bg-white/10 px-1 rounded">नीचे से ऊपर</span> पढ़ने पर मूल सर्वर → रिले सर्वर → प्राप्त सर्वर का पथ पता चलता है।',
    r4_ch8_l2: 'दुर्भावनापूर्ण बुनियादी ढाँचे की जाँच के लिए प्रेषण सर्वर IP को WHOIS और VirusTotal से खोजा जाता है।',
    r5_h: "नीति-आधारित रक्षा",
    r5_ch9: "अध्याय 9. GPO और ASR",
    r5_ch9_l1: '<span class="text-white bg-white/10 px-1 rounded">GPO (Group Policy Object)</span> — Active Directory के माध्यम से डोमेन-व्यापी सुरक्षा नीतियाँ तैनात करता है।',
    r5_ch9_l2: '<span class="text-white bg-white/10 px-1 rounded">ASR (Attack Surface Reduction)</span> — Office मैक्रो चाइल्ड प्रोसेस निर्माण, अस्पष्ट स्क्रिप्ट निष्पादन आदि को मूल स्रोत पर रोकता है।',
    r5_ch10: "अध्याय 10. त्रि-अक्ष रक्षा मॉडल",
    r5_ch10_l1: "प्रौद्योगिकी (Technology) — तकनीकी पहचान/अवरोधन उपकरण: Sysmon, YARA, Sigma, ASR",
    r5_ch10_l2: "प्रक्रिया (Process) — घटना प्रतिक्रिया प्रक्रियाएँ, पैच प्रबंधन, लॉग संरक्षण नीतियाँ",
    r5_ch10_l3: "लोग (People) — सुरक्षा जागरूकता प्रशिक्षण, फ़िशिंग सिमुलेशन, SOC विश्लेषक कौशल विकास",
    r6_title: "प्रशिक्षण पूर्ण!",
    r6_desc: "Sysmon निगरानी से YARA/Sigma पहचान नियम,<br/>ईमेल फोरेंसिक, और GPO/ASR नीति तैनाती तक —<br/>आपने मध्यवर्ती स्पीयरफ़िशिंग रक्षा पाठ्यक्रम पूरा किया।",
    r6_grade: "अंतिम ग्रेड:",
    r6_labBtn: "लैब में जाएँ",
    r6_freeHint: "(इस विंडो को बंद करके प्रत्येक सेक्टर को स्वतंत्र रूप से पुनः खोजें)",
  },
};
