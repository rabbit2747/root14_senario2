...
export const ScenarioSchema = z.object({
  ...
  safetyLevel: z.literal('defensive-education'),
  ...
  for (const step of scenario.steps) {
    for (const nodeId of step.activeNodes) {
      if (!nodeIds.has(nodeId)) {
        errors.push(`Step "${step.id}" references missing activeNode "${nodeId}".`)
      }
    }

    for (const edgeId of step.activeEdges) {
      if (!edgeIds.has(edgeId)) {
        errors.push(`Step "${step.id}" references missing activeEdge "${edgeId}".`)
      }
    }
